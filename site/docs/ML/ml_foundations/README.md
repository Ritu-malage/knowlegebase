# ML Foundations

Structured notes on AI/ML Foundations.  

The goal is to organize and document my understanding of classical machine learning concepts, models, and their applications. This includes fundamentals such as supervised and unsupervised learning, regression, classification, clustering, and evaluation metrics. The content will continue to evolve as I expand my knowledge into advanced techniques and practical use cases.

# Table of Contents
- [Introduction](./md_utils/introduction.md)
- [Data Preprocessing](./md_utils/data_preprocessing/data_preprocessing.md)
- [Models](./md_utils/models/models.md)
- [Performance Metrices](./md_utils/performance_metrices/performance_metrices.md)
- [Feature Extraction](./md_utils/feature_extraction/feature_extraction.md)
    

# References
- [https://www.udemy.com/course/machinelearning/](https://www.udemy.com/course/machinelearning/)