...
- It determines the **magnitude of the change that must be made to the weights and bias** during each step of the [gradient decent](#gradient-descent) process.
...
- Learning rate and [regularization](#regularization) rate often pull the weights in the opposite direction.
...
## 3. Batch Size
- Its a [hyperparameter](#hyper-parameters)
...
- Smaller batch size will behave like [SGD](#stochastic-gradient-descent) and larger size will behave like [full batch gradient descent](#full-batch).
...
- It lies between [full batch](#full-batch) and [Stochastic Gradient](#stochastic-gradient-descent)
...
# Exploding Gradient
- Using [gradient descent](#gradient-descent), we update model parameters such as weights and biases.  
...
- Regularization is a technique used to **reduce [overfitting](#overfitting)** and improve the **[generalization](#generalization)** ability of a model.  
...
- It **adds an absolute values of the weights as penalty to the loss function** i.e $\lambda \sum{|wi|}$ to the loss function 
- L1 regularization = $\frac{1}{n}\sum{(yi-yi')^2} + \lambda \sum{|wi|}$

### 2. L2 Regularization
- Also called as **Ridge Regularization**
- It adds an **Square values of the weights as penalty** to the loss function i.e $\lambda \sum{wi^2}$ to the loss function 
- L2 regularization = $\frac{1}{n}\sum{(yi-yi')^2} + \lambda \sum{wi^2}$
